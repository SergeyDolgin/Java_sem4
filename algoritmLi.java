...
// На поле MxN в точке (x, y) размещается некий объект. Имеется точка две точки выхода с координатами (exitX1, exitY1) и (exitX2, exitY2).
// Необходимо вывести на экран кратчайший маршрут, от точки старта до точки выхода (на экране от S до E)
// Некоторые ячейки являются препятствиями, остальные свободны. Размер поля, координаты старта и точек выхода запрашиваются у пользователя. Координаты препятствий определяются случайным образом.
...
public class algoritmLi {
    ...
    public static void main(String[] args) {
        ...
        if(SetMainParam()){
            ...
            System.out.printf("2. Объект находится в точке старта (%d, %d)\n",pointX, pointY);
    ...
    static void SetRoute(){
        ...
        for(int i=0; i < sizeY; i++ ){
            for(int j = 0; j < sizeX; j++){
                if(fieldWork[i][j] == -1) fieldChar[i][j] = '█';
            }
        }
    ...
    static boolean SetMainParam(){          
        System.out.println("Введите размеры поля (M x N), где M и N целые числа [1, 30]:");
    ...
    static int ValidField (String lit, int max){
        ...
        do {
            ...
            while (!inScan.hasNextInt()){
                System.out.printf("Пожалуйста, введите натуральное число от %d до %d.\n", min, max);
                System.out.print(lit + " = ");
                inScan.next();
            }
            inNum = inScan.nextInt();
            if(inNum  < min || inNum > max)  System.out.printf("Пожалуйста, введите натуральное число от %d до %d.\n", min, max);
